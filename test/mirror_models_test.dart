...
void main() {
  ...
  test('catalog publishes nine checksums with the Enterprise locale policy',
      () async {
    ...
    final simplifiedChinese = catalog
        .knownImagesForLocale(const Locale('zh'))
        .map((image) => image.id)
        .toSet();
    final traditionalChinese = catalog
        .knownImagesForLocale(const Locale('zh', 'TW'))
        .map((image) => image.id)
        .toSet();
    final english = catalog
        .knownImagesForLocale(const Locale('en'))
        .map((image) => image.id)
        .toSet();

    expect(simplifiedChinese.intersection(enterpriseIds), isEmpty);
    expect(traditionalChinese.intersection(enterpriseIds), isEmpty);
    expect(english.containsAll(enterpriseIds), isTrue);
    expect(simplifiedChinese, hasLength(7));
  ...
  test('catalog uses direct Global Mirror files and keeps excluded resources local',
    ...
    for (final id in directIds) {
      ...
      expect(
        url,
        contains('/project/windeploystudio/Extended%20Files/'),
      );
  ...
  test('English-only images expose language, time-zone, and font guidance',
      () async {
    final json = jsonDecode(await File('data/mirrors.json').readAsString())
        as Map<String, dynamic>;
    final catalog = MirrorListData.fromJson(json);
    ...
    expect(
      ...
      allOf(contains('english'), contains('language'), contains('time zone'), contains('font')),
    );
  });
}
