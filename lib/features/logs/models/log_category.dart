...
class LogStats {
  ...
  final Map<LogCategory, DateTime?> categoryLastUpdates;
  ...
  LogStats({
    ...
    required this.categoryLastUpdates,
  });
  ...
  String lastActivityFormatted(String Function(String) tr) {
    if (lastActivity == null) {
      return tr('logs_never');
    }
    final diff = DateTime.now().difference(lastActivity!);
    if (diff.inMinutes < 1) {
      return tr('logs_just_now');
    }
    if (diff.inMinutes < 60) {
      return tr('logs_min_ago').replaceAll('{n}', '${diff.inMinutes}');
    }
    if (diff.inHours < 24) {
      return tr('logs_hours_ago').replaceAll('{n}', '${diff.inHours}');
    }
