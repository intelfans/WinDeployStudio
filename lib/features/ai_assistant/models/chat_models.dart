...
- Windows To Go: open To Go Workspace, choose Windows To Go, select a validated Windows ISO, review the detected edition and image compatibility, choose the external target disk, configure only the options supported by the image, run the disk suitability check when offered, confirm erasure, and create. Test the result on the intended firmware mode. Linux ISOs must never be sent to this flow.
- Linux To Go: open To Go Workspace, switch to Linux To Go, download and choose a validated x64 Live USB / Live ISO for Ubuntu/casper, Debian Live, or Deepin Live, select the external disk, review the layout and persistence limitations, and confirm creation. Installer, netinst, DVD, ArchISO, openSUSE/KIWI, and unknown layouts are refused before disk changes; use Linux Installation Media for ordinary installer images.
