...
#define MyAppFileVersion "2.0.9.0"
...
OutputBaseFilename=WinDeployStudio_Setup_2.0.9
Compression=lzma2
...
korean.OpenSourceNoticeText=WinDeploy Studio는 MIT License에 따라 배포됩니다.%n%n타사 소프트웨어, 상표, 로고 및 외부 리소스는 각 소유자의 자산입니다.%n%n이 소프트웨어의 사용은 사용자 책임입니다.%n%n어떠한 보증도 제공되지 않습니다.%n%n이 릴리스에는 고정된 MIT 라이선스 go-ext4fs 소스에서 빌드한 제한된 ext4 영구 저장 이미지 도우미 wds_ext4_builder가 포함됩니다. Linux To Go 영구 저장은 아직 실험적이며 GPT 대상 디스크가 있는 x64 UEFI 시스템에서만 지원됩니다. UEFI+MBR 및 레거시 BIOS는 지원되지 않습니다. 구조 사전 검사를 통과한 x64 Ubuntu/casper, Debian Live/Kali 및 Deepin Live 프로필만 허용되며, 각 프로필은 실제 부팅과 재부팅 후 영구 저장 검증이 여전히 필요합니다. 설치 후 THIRD_PARTY_NOTICES.md와 tools\ext4-builder\README.md를 확인하세요.
...
japanese.OpenSourceNoticeText=WinDeploy Studio は MIT License の下で配布されています。%n%n第三者のソフトウェア、商標、ロゴ、外部リソースは、それぞれの所有者に帰属します。%n%n本ソフトウェアの使用は自己責任です。%n%nいかなる保証も提供されません。%n%nこのリリースには、固定された MIT ライセンスの go-ext4fs ソースから構築した制限付き ext4 永続化イメージヘルパー wds_ext4_builder が含まれます。Linux To Go の永続化はまだ実験的であり、GPT ターゲットディスクを使用する x64 UEFI システムでのみサポートされます。UEFI+MBR とレガシー BIOS はサポートされません。構造事前検査を通過した x64 Ubuntu/casper、Debian Live/Kali、Deepin Live プロファイルのみを対象とし、各プロファイルで実機起動と再起動後の永続化を検証する必要があります。インストール後に THIRD_PARTY_NOTICES.md と tools\ext4-builder\README.md を参照してください。
