...
#define MyAppFileVersion "2.1.0.0"
...
#define MyAppPrivacyPolicy "https://intelfans.github.io/WinDeployStudio/privacy.html"
...
OutputBaseFilename=WinDeployStudio_Setup_2.1.0
Compression=lzma2
...
Name: "disable_remote_ai"; Description: "{cm:DisableRemoteAi}"; GroupDescription: "{cm:PrivacyOptions}"; Flags: unchecked
...
english.OpenSourceNoticeText=WinDeploy Studio is distributed under the MIT License.%n%nThird-party software, trademarks, logos, and external resources remain the property of their respective owners.%n%nUse of this software is at your own risk.%n%nNo warranty is provided. See THIRD_PARTY_NOTICES.md after installation.
english.ViewMITLicense=View MIT License
english.ViewPrivacyPolicy=View Privacy Policy
english.PrivacyOptions=Privacy options:
english.DisableRemoteAi=Disable optional remote AI and web search (AI Assistant will remain unavailable until you rerun Setup and clear this option)
...
chinesesimplified.OpenSourceNoticeText=WinDeploy Studio 基于 MIT License 分发。%n%n第三方软件、商标、标志和外部资源仍归其各自所有者所有。%n%n使用本软件风险自负。%n%n本软件不提供任何担保。安装后请查看 THIRD_PARTY_NOTICES.md。
chinesesimplified.ViewMITLicense=查看 MIT License
chinesesimplified.ViewPrivacyPolicy=查看隐私政策
chinesesimplified.PrivacyOptions=隐私选项：
chinesesimplified.DisableRemoteAi=禁用可选的远程 AI 和联网搜索（AI 助手将不可用；重新运行安装程序并取消此选项后可恢复）
...
chinesetraditional.OpenSourceNoticeText=WinDeploy Studio 基於 MIT License 發行。%n%n第三方軟體、商標、標誌和外部資源仍歸其各自所有者所有。%n%n使用本軟體風險自負。%n%n本軟體不提供任何擔保。安裝後請查看 THIRD_PARTY_NOTICES.md。
chinesetraditional.ViewMITLicense=檢視 MIT License
chinesetraditional.ViewPrivacyPolicy=檢視隱私權政策
chinesetraditional.PrivacyOptions=隱私選項：
chinesetraditional.DisableRemoteAi=停用可選的遠端 AI 與網路搜尋（AI 助理將無法使用；重新執行安裝程式並取消此選項後可恢復）
...
russian.OpenSourceNoticeText=WinDeploy Studio распространяется по лицензии MIT.%n%nСтороннее ПО, товарные знаки, логотипы и внешние ресурсы остаются собственностью соответствующих владельцев.%n%nВы используете это ПО на свой риск.%n%nГарантии не предоставляются. После установки см. THIRD_PARTY_NOTICES.md.
russian.ViewMITLicense=Открыть лицензию MIT
russian.ViewPrivacyPolicy=Открыть политику конфиденциальности
russian.PrivacyOptions=Параметры конфиденциальности:
russian.DisableRemoteAi=Отключить удалённый AI и веб-поиск (AI останется недоступным, пока вы не запустите установщик снова и не снимете этот параметр)
...
french.OpenSourceNoticeText=WinDeploy Studio est distribué sous licence MIT.%n%nLes logiciels tiers, marques, logos et ressources externes restent la propriété de leurs détenteurs respectifs.%n%nVous utilisez ce logiciel à vos propres risques.%n%nAucune garantie n'est fournie. Consultez THIRD_PARTY_NOTICES.md après l'installation.
french.ViewMITLicense=Voir la licence MIT
french.ViewPrivacyPolicy=Voir la politique de confidentialité
french.PrivacyOptions=Options de confidentialité :
french.DisableRemoteAi=Désactiver l’IA distante et la recherche web facultatives (l’assistant IA restera indisponible jusqu’à ce que vous relanciez l’installation et retiriez cette option)
...
german.OpenSourceNoticeText=WinDeploy Studio wird unter der MIT-Lizenz verteilt.%n%nSoftware, Marken, Logos und externe Ressourcen Dritter bleiben Eigentum der jeweiligen Rechteinhaber.%n%nDie Nutzung dieser Software erfolgt auf eigenes Risiko.%n%nEs wird keine Gewährleistung übernommen. Lesen Sie nach der Installation THIRD_PARTY_NOTICES.md.
german.ViewMITLicense=MIT-Lizenz anzeigen
german.ViewPrivacyPolicy=Datenschutzerklärung anzeigen
german.PrivacyOptions=Datenschutzoptionen:
german.DisableRemoteAi=Optionale Remote-KI und Websuche deaktivieren (der KI-Assistent bleibt bis zum erneuten Setup ohne diese Option nicht verfügbar)
...
spanish.OpenSourceNoticeText=WinDeploy Studio se distribuye bajo la licencia MIT.%n%nEl software de terceros, las marcas, los logotipos y los recursos externos siguen siendo propiedad de sus respectivos propietarios.%n%nEl uso de este software es bajo su propio riesgo.%n%nNo se proporciona garantía. Consulta THIRD_PARTY_NOTICES.md tras la instalación.
spanish.ViewMITLicense=Ver licencia MIT
spanish.ViewPrivacyPolicy=Ver política de privacidad
spanish.PrivacyOptions=Opciones de privacidad:
spanish.DisableRemoteAi=Desactivar la IA remota y la búsqueda web opcionales (el Asistente de IA no estará disponible hasta que ejecutes de nuevo el instalador y desmarques esta opción)
...
portuguese.OpenSourceNoticeText=O WinDeploy Studio é distribuído sob a Licença MIT.%n%nSoftwares de terceiros, marcas, logotipos e recursos externos permanecem propriedade de seus respectivos proprietários.%n%nO uso deste software é por sua conta e risco.%n%nNenhuma garantia é fornecida. Consulte THIRD_PARTY_NOTICES.md após a instalação.
portuguese.ViewMITLicense=Ver Licença MIT
portuguese.ViewPrivacyPolicy=Ver política de privacidade
portuguese.PrivacyOptions=Opções de privacidade:
portuguese.DisableRemoteAi=Desativar a IA remota e a pesquisa na web opcionais (o Assistente de IA permanecerá indisponível até que execute novamente o instalador e desmarque esta opção)
...
arabic.OpenSourceNoticeText=يُوزَّع WinDeploy Studio بموجب ترخيص MIT.%n%nتبقى البرامج والعلامات التجارية والشعارات والموارد الخارجية التابعة لجهات خارجية ملكًا لأصحابها المعنيين.%n%nاستخدام هذا البرنامج يكون على مسؤوليتك الخاصة.%n%nلا يتم تقديم أي ضمان. راجع THIRD_PARTY_NOTICES.md بعد التثبيت.
arabic.ViewMITLicense=عرض ترخيص MIT
arabic.ViewPrivacyPolicy=عرض سياسة الخصوصية
arabic.PrivacyOptions=خيارات الخصوصية:
arabic.DisableRemoteAi=تعطيل الذكاء الاصطناعي عن بُعد والبحث على الويب الاختياريين (سيبقى مساعد AI غير متاح حتى تعيد تشغيل برنامج التثبيت وتزيل هذا الخيار)
...
korean.OpenSourceNoticeText=WinDeploy Studio는 MIT License에 따라 배포됩니다.%n%n타사 소프트웨어, 상표, 로고 및 외부 리소스는 각 소유자의 자산입니다.%n%n이 소프트웨어의 사용은 사용자 책임입니다.%n%n어떠한 보증도 제공되지 않습니다. 설치 후 THIRD_PARTY_NOTICES.md를 확인하세요.
korean.ViewMITLicense=MIT License 보기
korean.ViewPrivacyPolicy=개인정보 처리방침 보기
korean.PrivacyOptions=개인정보 보호 옵션:
korean.DisableRemoteAi=선택적 원격 AI 및 웹 검색 비활성화(AI 도우미는 설치 프로그램을 다시 실행하여 이 옵션을 해제할 때까지 사용할 수 없음)
...
japanese.OpenSourceNoticeText=WinDeploy Studio は MIT License の下で配布されています。%n%n第三者のソフトウェア、商標、ロゴ、外部リソースは、それぞれの所有者に帰属します。%n%n本ソフトウェアの使用は自己責任です。%n%nいかなる保証も提供されません。インストール後に THIRD_PARTY_NOTICES.md を参照してください。
japanese.ViewMITLicense=MIT License を表示
japanese.ViewPrivacyPolicy=プライバシーポリシーを表示
japanese.PrivacyOptions=プライバシー オプション:
japanese.DisableRemoteAi=オプションのリモート AI とウェブ検索を無効にする（セットアップを再実行してこの選択を解除するまで AI アシスタントは使用できません）
...
english.WelcomeLabel2=This will install WinDeploy Studio %1 on your computer.%n%nWinDeploy Studio creates Windows installation media, writes bootable Linux ISOHybrid images, builds portable Windows To Go workspaces, and includes native drive testing. Portable Linux workspaces are planned for a future release and are not available in this version.%n%nIt is recommended that you close all other applications before continuing.
...
chinesesimplified.WelcomeLabel2=安装程序将在您的计算机上安装 WinDeploy Studio %1。%n%nWinDeploy Studio 支持创建 Windows 安装盘、写入可启动的 Linux ISOHybrid 镜像、创建 Windows To Go，并提供原生磁盘测试。Linux 便携工作环境计划在未来版本中提供，当前版本暂不可用。%n%n建议您在继续之前关闭所有其他应用程序。
...
russian.WelcomeLabel2=Эта программа установит WinDeploy Studio %1 на ваш компьютер.%n%nWinDeploy Studio создаёт установочные носители Windows, записывает загрузочные Linux ISOHybrid-образы, создаёт переносные среды Windows To Go и тестирует накопители. Переносные рабочие среды Linux запланированы для будущей версии и сейчас недоступны.%n%nРекомендуется закрыть все остальные приложения перед продолжением.
...
french.WelcomeLabel2=Cette procédure va installer WinDeploy Studio %1 sur votre ordinateur.%n%nWinDeploy Studio crée des supports d’installation Windows, écrit des images Linux ISOHybrid amorçables, crée des environnements Windows To Go portables et propose un test natif des disques. Les espaces de travail Linux portables sont prévus pour une future version et ne sont pas disponibles actuellement.%n%nIl est recommandé de fermer toutes les autres applications avant de continuer.
...
portuguese.WelcomeLabel2=Este procedimento instalará o WinDeploy Studio %1 no seu computador.%n%nO WinDeploy Studio cria mídias de instalação do Windows, grava imagens Linux ISOHybrid inicializáveis, cria ambientes Windows To Go portáteis e oferece teste nativo de unidades. Ambientes Linux portáteis estão planejados para uma versão futura e não estão disponíveis agora.%n%nRecomenda-se fechar todos os outros aplicativos antes de continuar.
...
japanese.WelcomeLabel2=このプログラムは WinDeploy Studio %1 をお使いのコンピューターにインストールします。%n%nWinDeploy Studio は Windows インストールメディアの作成、起動可能な Linux ISOHybrid の書き込み、ポータブルな Windows To Go、ネイティブなドライブテストを提供します。Linux のポータブルワークスペースは将来のリリースで提供予定で、現在のバージョンでは利用できません。%n%n続行する前に、他のすべてのアプリケーションを閉じることをお勧めします。
...
chinesetraditional.WelcomeLabel2=安裝程式將在您的電腦上安裝 WinDeploy Studio %1。%n%nWinDeploy Studio 支援建立 Windows 安裝碟、寫入可啟動的 Linux ISOHybrid 映像、建立 Windows To Go，並提供原生磁碟測試。Linux 可攜式工作環境計劃在未來版本中提供，目前版本暫不可用。%n%n建議您在繼續之前關閉所有其他應用程式。
...
spanish.WelcomeLabel2=Este procedimiento instalará WinDeploy Studio %1 en su equipo.%n%nWinDeploy Studio crea medios de instalación de Windows, escribe imágenes Linux ISOHybrid arrancables, crea entornos Windows To Go portátiles e incluye pruebas nativas de unidades. Los espacios de trabajo Linux portátiles están previstos para una versión futura y no están disponibles actualmente.%n%nSe recomienda cerrar todas las demás aplicaciones antes de continuar.
...
arabic.WelcomeLabel2=سيقوم هذا المثبت بتثبيت WinDeploy Studio %1 على جهازك.%n%nينشئ WinDeploy Studio وسائط تثبيت Windows، ويكتب صور Linux ISOHybrid قابلة للإقلاع، وينشئ بيئات Windows To Go محمولة، ويوفر اختباراً أصلياً لمحركات الأقراص. بيئات Linux المحمولة مخطط لها في إصدار مستقبلي وليست متاحة حالياً.%n%nيُوصى بإغلاق جميع التطبيقات الأخرى قبل المتابعة.
...
korean.WelcomeLabel2=이 프로그램은 WinDeploy Studio %1을(를) 컴퓨터에 설치합니다.%n%nWinDeploy Studio는 Windows 설치 미디어를 만들고 부팅 가능한 Linux ISOHybrid 이미지를 쓰며, 휴대용 Windows To Go와 네이티브 드라이브 테스트를 제공합니다. Linux 휴대용 작업 환경은 향후 릴리스에서 제공될 예정이며 현재 버전에서는 사용할 수 없습니다.%n%n계속하기 전에 다른 모든 애플리케이션을 닫는 것이 좋습니다.
...
var
  ...
  ViewPrivacyPolicyButton: TNewButton;
...
procedure ViewPrivacyPolicyButtonClick(Sender: TObject);
var
  ResultCode: Integer;
begin
  ShellExec('', '{#MyAppPrivacyPolicy}', '', '', SW_SHOWNORMAL, ewNoWait, ResultCode);
end;
...
begin
  ...
  MarkerPath := ExpandConstant('{app}\\remote-ai-disabled.flag');
  if WizardIsTaskSelected('disable_remote_ai') then
    SaveStringToFile(
      MarkerPath,
      'Remote AI and web search disabled by installer privacy option.' + #13#10,
      False
    )
...
begin
  ...
  OpenSourceNoticeMemo.Height := OpenSourceNoticePage.SurfaceHeight - ScaleY(72);
  ...
  ViewPrivacyPolicyButton := TNewButton.Create(OpenSourceNoticePage);
  ViewPrivacyPolicyButton.Parent := OpenSourceNoticePage.Surface;
  ViewPrivacyPolicyButton.Left := ViewMITLicenseButton.Left + ViewMITLicenseButton.Width + ScaleX(8);
  ViewPrivacyPolicyButton.Top := ViewMITLicenseButton.Top;
  ViewPrivacyPolicyButton.Width := ScaleX(170);
  ViewPrivacyPolicyButton.Height := ScaleY(24);
  ViewPrivacyPolicyButton.Caption := CustomMessage('ViewPrivacyPolicy');
  ViewPrivacyPolicyButton.OnClick := @ViewPrivacyPolicyButtonClick;
end;
