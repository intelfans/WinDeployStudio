...
chinesesimplified.OpenSourceNoticeText=WinDeploy Studio 基于 MIT License 分发。%n%n第三方软件、商标、标志和外部资源仍归其各自所有者所有。%n%n使用本软件风险自负。%n%n本软件不提供任何担保。%n%n本发行版未内置 ext4 持久化工具。持久化 Linux To Go 创建会保持关闭，直到能随工具分发 GPLv2 所需的完整对应源码包。安装后请查看 THIRD_PARTY_NOTICES.md。
...
chinesetraditional.OpenSourceNoticeText=WinDeploy Studio 基於 MIT License 發行。%n%n第三方軟體、商標、標誌和外部資源仍歸其各自所有者所有。%n%n使用本軟體風險自負。%n%n本軟體不提供任何擔保。%n%n本發行版未內建 ext4 持久化工具。持久化 Linux To Go 建立會保持關閉，直到能隨工具發行 GPLv2 所需的完整對應原始碼套件。安裝後請查看 THIRD_PARTY_NOTICES.md。
...
russian.OpenSourceNoticeText=WinDeploy Studio распространяется по лицензии MIT.%n%nСтороннее ПО, товарные знаки, логотипы и внешние ресурсы остаются собственностью соответствующих владельцев.%n%nВы используете это ПО на свой риск.%n%nГарантии не предоставляются.%n%nЭта версия не содержит инструмента постоянного хранения ext4. Создание постоянного Linux To Go отключено, пока вместе с инструментом нельзя распространять полный соответствующий исходный пакет GPLv2. После установки см. THIRD_PARTY_NOTICES.md.
...
german.OpenSourceNoticeText=WinDeploy Studio wird unter der MIT-Lizenz verteilt.%n%nSoftware, Marken, Logos und externe Ressourcen Dritter bleiben Eigentum der jeweiligen Rechteinhaber.%n%nDie Nutzung dieser Software erfolgt auf eigenes Risiko.%n%nEs wird keine Gewährleistung übernommen.%n%nDiese Version enthält kein ext4-Persistenztool. Die Erstellung persistenter Linux-To-Go-Umgebungen bleibt deaktiviert, bis das vollständige GPLv2-Quellpaket zusammen mit dem Tool verteilt werden kann. Lesen Sie nach der Installation THIRD_PARTY_NOTICES.md.
...
spanish.OpenSourceNoticeText=WinDeploy Studio se distribuye bajo la licencia MIT.%n%nEl software de terceros, las marcas, los logotipos y los recursos externos siguen siendo propiedad de sus respectivos propietarios.%n%nEl uso de este software es bajo su propio riesgo.%n%nNo se proporciona garantía.%n%nEsta versión no incluye una herramienta de persistencia ext4. La creación de Linux To Go persistente permanece desactivada hasta que el paquete de código fuente correspondiente GPLv2 completo se pueda distribuir con la herramienta. Consulta THIRD_PARTY_NOTICES.md tras la instalación.
...
portuguese.OpenSourceNoticeText=O WinDeploy Studio é distribuído sob a Licença MIT.%n%nSoftwares de terceiros, marcas, logotipos e recursos externos permanecem propriedade de seus respectivos proprietários.%n%nO uso deste software é por sua conta e risco.%n%nNenhuma garantia é fornecida.%n%nEsta versão não inclui uma ferramenta de persistência ext4. A criação persistente de Linux To Go permanece desativada até que o pacote completo de código-fonte correspondente GPLv2 possa ser distribuído com a ferramenta. Consulte THIRD_PARTY_NOTICES.md após a instalação.
...
arabic.OpenSourceNoticeText=يُوزَّع WinDeploy Studio بموجب ترخيص MIT.%n%nتبقى البرامج والعلامات التجارية والشعارات والموارد الخارجية التابعة لجهات خارجية ملكًا لأصحابها المعنيين.%n%nاستخدام هذا البرنامج يكون على مسؤوليتك الخاصة.%n%nلا يتم تقديم أي ضمان.%n%nلا يتضمن هذا الإصدار أداة استمرارية ext4. يبقى إنشاء Linux To Go الدائم معطلاً حتى يمكن توزيع حزمة المصدر المطابق الكاملة لـ GPLv2 مع الأداة. راجع THIRD_PARTY_NOTICES.md بعد التثبيت.
...
korean.OpenSourceNoticeText=WinDeploy Studio는 MIT License에 따라 배포됩니다.%n%n타사 소프트웨어, 상표, 로고 및 외부 리소스는 각 소유자의 자산입니다.%n%n이 소프트웨어의 사용은 사용자 책임입니다.%n%n어떠한 보증도 제공되지 않습니다.%n%n이 릴리스에는 ext4 영구 저장 도구가 포함되어 있지 않습니다. 도구와 함께 완전한 GPLv2 대응 소스 패키지를 배포할 수 있을 때까지 영구 Linux To Go 생성은 비활성화됩니다. 설치 후 THIRD_PARTY_NOTICES.md를 확인하세요.
...
japanese.OpenSourceNoticeText=WinDeploy Studio は MIT License の下で配布されています。%n%n第三者のソフトウェア、商標、ロゴ、外部リソースは、それぞれの所有者に帰属します。%n%n本ソフトウェアの使用は自己責任です。%n%nいかなる保証も提供されません。%n%nこのリリースには ext4 永続化ツールは同梱されていません。ツールとともに完全な GPLv2 対応ソースパッケージを配布できるまで、永続化 Linux To Go の作成は無効です。インストール後に THIRD_PARTY_NOTICES.md を参照してください。
...
english.WelcomeLabel2=This will install WinDeploy Studio %1 on your computer.%n%nWinDeploy Studio creates Windows installation media, writes bootable Linux ISOHybrid images, builds portable Windows To Go workspaces, validates supported x64 Ubuntu/casper and Debian Live layouts, and includes native drive testing. Persistent Linux To Go creation is disabled in this release pending a compliant ext4 tool package.%n%nIt is recommended that you close all other applications before continuing.
...
chinesesimplified.WelcomeLabel2=安装程序将在您的计算机上安装 WinDeploy Studio %1。%n%nWinDeploy Studio 支持创建 Windows 安装盘、写入可启动的 Linux ISOHybrid 镜像、创建 Windows To Go、验证受支持的 x64 Ubuntu/casper 与 Debian Live 布局，并提供原生磁盘测试。本发行版会在合规 ext4 工具包就绪前关闭持久化 Linux To Go 创建。%n%n建议您在继续之前关闭所有其他应用程序。
...
russian.WelcomeLabel2=Эта программа установит WinDeploy Studio %1 на ваш компьютер.%n%nWinDeploy Studio создаёт установочные носители Windows, записывает загрузочные Linux ISOHybrid-образы, создаёт переносные среды Windows To Go, проверяет поддерживаемые x64-разметки Ubuntu/casper и Debian Live, а также тестирует накопители. Создание постоянного Linux To Go отключено до появления соответствующего требованиям ext4-пакета.%n%nРекомендуется закрыть все остальные приложения перед продолжением.
...
portuguese.WelcomeLabel2=Este procedimento instalará o WinDeploy Studio %1 no seu computador.%n%nO WinDeploy Studio cria mídias de instalação do Windows, grava imagens Linux ISOHybrid inicializáveis, cria ambientes Windows To Go portáteis, valida layouts compatíveis Ubuntu/casper e Debian Live x64 e oferece teste nativo de unidades. A criação persistente de Linux To Go está desativada até que exista um pacote ext4 compatível.%n%nRecomenda-se fechar todos os outros aplicativos antes de continuar.
...
japanese.WelcomeLabel2=このプログラムは WinDeploy Studio %1 をお使いのコンピューターにインストールします。%n%nWinDeploy Studio は Windows インストールメディアの作成、起動可能な Linux ISOHybrid の書き込み、ポータブルな Windows To Go、対応する x64 Ubuntu/casper と Debian Live レイアウトの検証、ネイティブなドライブテストを提供します。永続化 Linux To Go の作成は、準拠した ext4 パッケージが用意されるまで無効です。%n%n続行する前に、他のすべてのアプリケーションを閉じることをお勧めします。
...
chinesetraditional.WelcomeLabel2=安裝程式將在您的電腦上安裝 WinDeploy Studio %1。%n%nWinDeploy Studio 支援建立 Windows 安裝碟、寫入可啟動的 Linux ISOHybrid 映像、建立 Windows To Go、驗證受支援的 x64 Ubuntu/casper 與 Debian Live 配置，並提供原生磁碟測試。本發行版會在合規 ext4 工具套件就緒前關閉持久化 Linux To Go 建立。%n%n建議您在繼續之前關閉所有其他應用程式。
...
spanish.WelcomeLabel2=Este procedimiento instalará WinDeploy Studio %1 en su equipo.%n%nWinDeploy Studio crea medios de instalación de Windows, escribe imágenes Linux ISOHybrid arrancables, crea entornos Windows To Go portátiles, valida diseños compatibles Ubuntu/casper y Debian Live x64 e incluye pruebas nativas de unidades. La creación de Linux To Go persistente está desactivada hasta disponer de un paquete ext4 conforme.%n%nSe recomienda cerrar todas las demás aplicaciones antes de continuar.
...
arabic.WelcomeLabel2=سيقوم هذا المثبت بتثبيت WinDeploy Studio %1 على جهازك.%n%nينشئ WinDeploy Studio وسائط تثبيت Windows، ويكتب صور Linux ISOHybrid قابلة للإقلاع، وينشئ بيئات Windows To Go محمولة، ويتحقق من تخطيطات x64 Ubuntu/casper وDebian Live المدعومة، ويوفر اختباراً أصلياً لمحركات الأقراص. إنشاء Linux To Go الدائم معطّل حتى تتوفر حزمة ext4 متوافقة.%n%nيُوصى بإغلاق جميع التطبيقات الأخرى قبل المتابعة.
...
korean.WelcomeLabel2=이 프로그램은 WinDeploy Studio %1을(를) 컴퓨터에 설치합니다.%n%nWinDeploy Studio는 Windows 설치 미디어를 만들고 부팅 가능한 Linux ISOHybrid 이미지를 쓰며, 휴대용 Windows To Go, 지원되는 x64 Ubuntu/casper 및 Debian Live 레이아웃 검증, 네이티브 드라이브 테스트를 제공합니다. 영구 Linux To Go 생성은 준수 가능한 ext4 패키지가 준비될 때까지 비활성화됩니다.%n%n계속하기 전에 다른 모든 애플리케이션을 닫는 것이 좋습니다.
