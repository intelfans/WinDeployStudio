...
english.OpenSourceNoticeText=WinDeploy Studio is distributed under the MIT License.%n%nThird-party software, trademarks, logos, and external resources remain the property of their respective owners.%n%nUse of this software is at your own risk.%n%nNo warranty is provided.%n%nLinux To Go includes a bundled mke2fs.exe utility from e2fsprogs/AOSP to create ext4 persistence images. It is invoked as a separate command-line tool and remains third-party software.
...
chinesesimplified.OpenSourceNoticeText=WinDeploy Studio 基于 MIT License 分发。%n%n第三方软件、商标、标志和外部资源仍归其各自所有者所有。%n%n使用本软件风险自负。%n%n本软件不提供任何担保。%n%nLinux To Go 随程序内置来自 e2fsprogs/AOSP 的 mke2fs.exe 工具，用于创建 ext4 持久化镜像。该工具以独立命令行程序调用，仍属于第三方软件。
...
chinesetraditional.OpenSourceNoticeText=WinDeploy Studio 基於 MIT License 發行。%n%n第三方軟體、商標、標誌和外部資源仍歸其各自所有者所有。%n%n使用本軟體風險自負。%n%n本軟體不提供任何擔保。%n%nLinux To Go 隨程式內建來自 e2fsprogs/AOSP 的 mke2fs.exe 工具，用於建立 ext4 持久化映像。該工具以獨立命令列程式呼叫，仍屬於第三方軟體。
...
russian.OpenSourceNoticeText=WinDeploy Studio распространяется по лицензии MIT.%n%nСтороннее ПО, товарные знаки, логотипы и внешние ресурсы остаются собственностью соответствующих владельцев.%n%nВы используете это ПО на свой риск.%n%nГарантии не предоставляются.%n%nLinux To Go включает mke2fs.exe из e2fsprogs/AOSP для создания ext4-образов постоянного хранения. Он запускается как отдельная консольная утилита и остается сторонним ПО.
...
french.OpenSourceNoticeText=WinDeploy Studio est distribué sous licence MIT.%n%nLes logiciels tiers, marques, logos et ressources externes restent la propriété de leurs détenteurs respectifs.%n%nVous utilisez ce logiciel à vos propres risques.%n%nAucune garantie n'est fournie.%n%nLinux To Go inclut l'utilitaire mke2fs.exe fourni avec e2fsprogs/AOSP pour créer des images de persistance ext4. Il est lancé comme outil en ligne de commande séparé et reste un logiciel tiers.
...
spanish.OpenSourceNoticeText=WinDeploy Studio se distribuye bajo la licencia MIT.%n%nEl software de terceros, las marcas, los logotipos y los recursos externos siguen siendo propiedad de sus respectivos propietarios.%n%nEl uso de este software es bajo su propio riesgo.%n%nNo se proporciona garantía.%n%nLinux To Go incluye la utilidad mke2fs.exe de e2fsprogs/AOSP para crear imágenes de persistencia ext4. Se ejecuta como una herramienta de línea de comandos independiente y sigue siendo software de terceros.
...
portuguese.OpenSourceNoticeText=O WinDeploy Studio é distribuído sob a Licença MIT.%n%nSoftwares de terceiros, marcas, logotipos e recursos externos permanecem propriedade de seus respectivos proprietários.%n%nO uso deste software é por sua conta e risco.%n%nNenhuma garantia é fornecida.%n%nO Linux To Go inclui o utilitário mke2fs.exe de e2fsprogs/AOSP para criar imagens de persistência ext4. Ele é executado como uma ferramenta de linha de comando separada e continua sendo software de terceiros.
...
arabic.OpenSourceNoticeText=يُوزَّع WinDeploy Studio بموجب ترخيص MIT.%n%nتبقى البرامج والعلامات التجارية والشعارات والموارد الخارجية التابعة لجهات خارجية ملكًا لأصحابها المعنيين.%n%nاستخدام هذا البرنامج يكون على مسؤوليتك الخاصة.%n%nلا يتم تقديم أي ضمان.%n%nيتضمن Linux To Go أداة mke2fs.exe المرفقة من e2fsprogs/AOSP لإنشاء صور استمرارية ext4. يتم تشغيلها كأداة سطر أوامر منفصلة وتبقى برنامجًا تابعًا لجهة خارجية.
...
korean.OpenSourceNoticeText=WinDeploy Studio는 MIT License에 따라 배포됩니다.%n%n타사 소프트웨어, 상표, 로고 및 외부 리소스는 각 소유자의 자산입니다.%n%n이 소프트웨어의 사용은 사용자 책임입니다.%n%n어떠한 보증도 제공되지 않습니다.%n%nLinux To Go에는 ext4 지속성 이미지를 만들기 위한 e2fsprogs/AOSP 기반 mke2fs.exe 유틸리티가 포함되어 있습니다. 이 도구는 별도의 명령줄 도구로 실행되며 타사 소프트웨어입니다.
...
japanese.OpenSourceNoticeText=WinDeploy Studio は MIT License の下で配布されています。%n%n第三者のソフトウェア、商標、ロゴ、外部リソースは、それぞれの所有者に帰属します。%n%n本ソフトウェアの使用は自己責任です。%n%nいかなる保証も提供されません。%n%nLinux To Go には、ext4 永続化イメージを作成するために e2fsprogs/AOSP 由来の mke2fs.exe ユーティリティが同梱されています。これは別個のコマンドラインツールとして実行され、第三者ソフトウェアのままです。
